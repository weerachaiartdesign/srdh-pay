  ...
  container.innerHTML = `
    <div class="container-fluid fade-in">
      <h4 class="mb-4 text-purple fw-bold d-flex align-items-center gap-2">
        <i class="bi bi-pie-chart-fill"></i>รายงานการเบิกจ่ายเงิน
      </h4>
...
function drawDashboardContent() {
  const mTypes = ["เงินงบประมาณ", "เงินบำรุง", "เงินประกันสุขภาพ", "เงินอุดหนุน", "เงินอื่น"];
  let html = '';
  ...
  document.getElementById('avg-time').innerHTML = `🕒 ระยะเวลาการดำเนินการเฉลี่ย (รับเรื่อง - อนุมัติ) : <span class="text-purple fw-bold fs-5 ms-2">${avg} วัน</span>`;

  // กราฟที่ 1: สถิติการเบิกจ่าย
  const depts = ["กลุ่มงานเภสัชกรรม", "ฝ่ายพัสดุ", "ฝ่ายโภชนาการ", "กลุ่มงานประกันสุขภาพ", "ฝ่ายบริหารงานทั่วไป", "กลุ่มงานธนาคารเลือด", "กลุ่มงานเทคนิคการแพทย์", "กลุ่มงานรังสีวิทยา", "อื่นๆ"];
    ...
    new Chart(ctx1, {
      ...
      data: {
        ...
        datasets: [
          ...
          { label: 'เงิน (บาท)', data: depts.map(n => allData.filter(d => d.dept && d.dept.includes(n)).reduce((a,b)=>a+(Number(b.amount)||0),0)/100000), backgroundColor: '#E0B0FF', borderRadius: 5 }
        ]
      },
      options: { responsive: true, plugins: { title: { display: true, text: 'สถิติการเบิกจ่าย' } } }
    });
  }

  // กราฟที่ 2: สัดส่วนสถานะ
  const statuses = ["รับเข้าระบบ", "ตรวจสอบ", "ส่งแก้ไข", "ผ่านการตรวจ", "ส่งเสนอ", "อนุมัติจ่าย", "รอจ่าย", "จ่ายแล้ว", "ยกเลิก"];
