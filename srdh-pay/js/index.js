...
/**
 * ฟังก์ชัน init:
 * ดึงข้อมูลจาก API โดยอ้างอิงค่าจาก CONFIG ใน js/api-config.js
...
async function init() {
  ...
  try {
    ...
    if (typeof CONFIG === 'undefined') {
      throw new Error("ไม่พบไฟล์ตั้งค่าระบบ (js/api-config.js)");
    }
...
function changePage(page) {
  ...
  if(page === 'dashboard') {
    ...
    // เรียกฟังก์ชันจาก js/dashboard.js
    ...
    fetch('register-list.html')
      ...
      .catch(err => {
        container.innerHTML = `<div class="alert alert-warning">โหลดรายการไม่สำเร็จ: ${err.message}</div>`;
      });
  }
}
